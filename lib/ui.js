...
function createStatusPanel(options, clickCallBack) {
  const defaults = {
    boxLayout: {
  ...
  const container = new PanelMenu.Button(0.0);
  const gicon = Gio.icon_new_for_string(parameters.iconFile);

  const section = new createMenuSection();
  const hbox = new St.BoxLayout({ style_class: parameters.icon.style_class });
